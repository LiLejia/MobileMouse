...
- (BOOL)sendString:(NSString *)data error:(NSError **)error{
    NSLog(@"d");
    
    BOOL successful = NO;
    if(self.outputStreamHasSpace){
        
        const unsigned char *dataString = (const unsigned char *)[data UTF8String];
        
        NSInteger len = [self.outputStream write:dataString maxLength:[data lengthOfBytesUsingEncoding:NSUTF8StringEncoding]];
